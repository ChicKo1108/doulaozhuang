...
- 系统支持 Mard 24、48、72、96、120、221 色标准用色档位；221 色为默认档位和母色卡。
- 用户在转图时选择用色档位，首次默认选择 221 色；MVP 中小档位按图片主色在 Mard 母色卡内二次量化，保证图纸不会超过所选色数。
...
1. **颜色标准与匹配：** Mard 221 色为系统母色卡；图纸转换必须限制在用户选择的 `24 / 48 / 72 / 96 / 120 / 221 色`用色档位内，默认 221 色。图纸、库存、预占和补货均以 `mard_color_code` 为唯一颜色键。算法可计算近似色，但不能输出母色卡外颜色或超过所选色数。
