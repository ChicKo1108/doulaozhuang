...
2. 豆子库：从内置的 Mard 221 色中选择颜色，新增、盘点和排序库存；记录 Mard 色号、当前数量、预警数量。
3. 图纸库：上传图片并自动转换为拼豆图纸；按 Mard 色号筛选；查看每色用量、总用豆数和完成状态。
...
| 字段 | 说明 | 必填 |
| --- | --- | --- |
| Mard 色号 | 从内置 Mard 221 色标准色卡中选择，系统唯一标识 | 是 |
...
- Mard 221 色卡为首期唯一颜色标准；同一用户每个 Mard 色号仅有一条有效库存记录。
...
1. **已有拼豆图纸图片：** 例如他人分享的 PNG/JPG 图纸。系统识别其网格与颜色，转换为 Mard 221 色图纸并统计用量。
2. **原始图片：** 例如照片、插画或表情图。系统调用既有图纸转换算法，完成尺寸/格数处理、Mard 221 色映射、网格图生成和用量统计。

转换完成后进入“转换结果确认页”，展示原图、生成图纸预览、成品格数、颜色数、总用豆数和逐色用量。用户可修改图纸名称、选择预设尺寸/格数后重新转换并确认入库；首期不提供从零手工新建图纸或手工填写色表。
...
1. **颜色标准与匹配：** 图纸转换算法的输出必须映射到 Mard 221 色标准；图纸、库存、预占和补货均以 `mard_color_code` 为唯一颜色键。算法可计算近似色，但不能输出色卡外颜色。
...
```mermaid
flowchart LR
  A[上传原始图片或已有图纸图片] --> B[自动转换为 Mard 221 色图纸]
  B --> C[确认转换结果并入库]
  C --> D{库存是否充足}
  D -->|是| E[开始拼：预占库存]
  D -->|否| F[生成补豆清单]
